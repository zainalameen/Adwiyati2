...
final todayProgressProvider = Provider.autoDispose<Map<String, dynamic>>((ref) {
  final dosesAsync = ref.watch(dosesForDateProvider);
  return dosesAsync.when(
    data: (doses) {
      final total = doses.length;
      final taken = doses
          .where((d) => d.status == DoseReminderStatus.taken)
          .length;
    ...
    error: (_, _) => {'taken': 0, 'total': 0, 'adherence': 0},
  );
});

final overallStatsProvider = FutureProvider.autoDispose<Map<String, dynamic>>((
  ref,
) async {
  final user = SupabaseService.auth.currentUser;
  if (user == null) {
